...
// 弹出框标题
const dialogTitle = computed(() => dialogType.value === 'create' ? '创建角色' : '编辑角色')
// 弹出框表单内容
const form = reactive({
  obj: {
    name: '',
  ...
  <div id="sanguo">
    <div class="head">
      ...
      <div class="year">
        ...
        <el-select v-model="selectedYear" class="m-2" placeholder="Select" size="default">
          <el-option v-for="item in yearList" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
        <el-button @click="digitYear('plus')">+</el-button>
      </div>
      <div class="search">
        <el-input v-model="searchValue" placeholder="请输入角色姓名" :prefix-icon="Search" />
    ...
    <div class="main">
      <div class="left-main">
        <div class="year-zone">
          <div class="year-box">
            <YearCard :sanguoYearList="sanguoYearList" :selectedYear="selectedYear" @changeYear="changeYear" />
          </div>
        </div>
      </div>
      <div class="right-main">
        <div class="view-zone">
          <div class="view-container">
            <ViewCard v-for="(item, index) in viewList" :selectedYear="selectedYear" :data="item" :key="item._id"
              @outCharacter="outCharacter(item)" />
          </div>
          <div class="sort-container">
            <!-- 引发报错？Uncaught ReferenceError: browser is not defined -->
            <el-select v-model="sortType" placeholder="排序方式" @change="changeSort">
              <el-option v-for="(item, index) in sortOptions" :key="item.value + index" :label="item.label"
                :value="item.value" />
            </el-select>
          </div>
          <div class="clear-all">
        ...
        <div class="search-zone">
          <div class="search-container">
            <SearchCard v-for="item in searchList" :data="item" :key="item._id" @putCharacter="putCharacter(item)"
              @editCharacter="editCharacter(item)" @deleteCharacter="deleteCharacter(item)" />
          </div>
        </div>
      </div>
    </div>
